...
# Per-dialect type maps: Informatica datatype -> native DDL type
...
# Per-dialect dbt profile stanzas
        ...
        azure_synapse_profile:
          ...
          outputs:
            dev:
              ...
              database: "<database>"
              schema: "<schema>"
              authentication: ActiveDirectoryInteractive   # or sql / ServicePrincipal
              client_id: "<client_id>"
        ...
        mssql_profile:
          ...
          outputs:
            dev:
              ...
              database: "<database>"
              schema: "<schema>"
              authentication: sql               # sql | ActiveDirectoryPassword | ActiveDirectoryMSI
              username: "<user>"
              password: "<password>"
              threads: 4
            prod:
        ...
        oracle_profile:
          ...
          outputs:
            dev:
              ...
              host: "<host>"
              port: 1521
              database: "<service_name>"        # Oracle service name
              schema: "<schema>"
              threads: 4
            prod:
              type: oracle
...
# Per-dialect dbt adapter pip package
...
# Per-dialect SQL quoting / date functions used in generated models
...
def dialect_type(infa_type: str, dialect: str) -> str:
    """Return the native DDL type for a given Informatica datatype and target dialect."""
...
class TransformField:
    ...
    def native_type(self, dialect: str = "duckdb") -> str:
        """Return DDL type for the given target dialect."""
...
class Mapping:
    ...
    def topo_sort(self) -> list[str]:
        # Build dependency map only over known instances
        deps: dict[str, set] = {n: set() for n in self.instances}
        for c in self.connectors:
            fi = c.from_instance
            ti = c.to_instance
            # Force both endpoints to be present before building the edge
            if fi not in deps:
                deps[fi] = set()
            if ti not in deps:
                deps[ti] = set()
    ...
    def _ttype(self, inst_name: str) -> str:
        """Safe transformation type lookup — handles reusable/external instances."""
    ...
    def detect_source_target_pairs(self):
        ...
        sources = [n for n in self.instances
                   if self._ttype(n) in
                   ("Input Transformation","Source Qualifier","Source Definition")]
        targets = [n for n in self.instances
                   if self._ttype(n) in
...
class PowerMartParser:
    ...
    def _parse_mapping_el(self, el: ET.Element, folder: str, is_mapplet: bool) -> Optional[Mapping]:
        ...
        # Each CONNECTOR element is evaluated individually so that FROMINSTANCE
        # and TOINSTANCE are resolved eagerly per line, regardless of order.
        # Unknown endpoints are registered as stub instances so downstream
        # topo-sort and CTE generation never encounter a missing key.
        for c_el in el.findall("CONNECTOR"):
            ...
            # Force resolution: register any endpoint that has no INSTANCE element
            # (happens with reusable or cross-folder transformations)
            for inst_name, trans_name, trans_type in (
                ...
                if inst_name and inst_name not in m.instances:
                    m.instances[inst_name] = MappingInstance(
                        ...
                        reusable=True,   # must be external/reusable if not in local INSTANCE list
                    )

            # Only append connectors where both endpoints are non-empty
...
class DbtGenerator:
    ...
    def __init__(self, out_dir: str = "./dbt_output",
        ...
        self.src_dialect = src_dialect   # where source data lives
        self.tgt_dialect = tgt_dialect   # where dbt models materialise
    ...
    def _write_profiles(self, connections=None):
        ...
        # Also write a separate source_profiles.yml if source != target
    ...
    def _ics_cols(self) -> dict[str, str]:
        """ICS audit columns in the target dialect's native types."""
        src_map = {
            "Object_Name":"string",    "Run_Id":"bigint",
            "Log_Id":"bigint",         "Task_Id":"string",
            "Task_Status":"string",    "Success_Source_Rows":"bigint",
            "Failed_Source_Rows":"bigint","Success_Target_Rows":"bigint",
            "Failed_Target_Rows":"bigint","Start_Time":"date/time",
            "End_Time":"date/time",    "Error_Message":"string",
            "TotalTransErrors":"bigint","FirstErrorCode":"string",
        }
        return {col: dialect_type(infa_t, self.tgt_dialect)
                for col, infa_t in src_map.items()}
        ...
        for inst_name in reversed(m.topo_sort()):
            ...
            if t and t.ttype in ("Output Transformation","Target Definition"):
                return [{"name": f.name.lower(),
                          "dtype": f.native_type(self.tgt_dialect),
    ...
    def _write_seed_ddl(self, taskflows, mappings, workflows):
        ...
        blocks   = [
            f"-- Auto-generated DDL\n"
            f"-- Source dialect: {src_d}  |  Target dialect: {tgt_d}\n\n",
        ]
        # Source raw tables  (use src_dialect types)
        blocks.append("-- ===== SOURCE RAW TABLES =====\n")
        for m in mappings:
            for inst_name, inst in m.instances.items():
                t = m.transformations.get(inst.transformation_name)
                if t and t.ttype in ("Input Transformation","Source Qualifier","Source Definition"):
                    ...
                    col_block = ",\n    ".join(
                        f"{f.name.lower()}  {f.native_type(src_d)}" for f in t.outputs
                    ) or "id  integer"
                    blocks.append(
        ...
        # Target tables  (use tgt_dialect types)
        blocks.append("\n-- ===== TARGET TABLES (dbt materialisation targets) =====\n")
        for m in mappings:
            for pair in m.source_target_pairs:
                ...
                col_block = ",\n    ".join(
                    f"{f.name.lower()}  {f.native_type(tgt_d)}" for f in tgt_trans.inputs
                ...
                blocks.append(
                    f"-- Target: {pair.target_table}  {'[SCD2]' if pair.is_scd2 else ''}\n"
                    f"CREATE TABLE IF NOT EXISTS {tgt_tbl} (\n"
    ...
    def _db(self):
        ...
        for row in mapping_rows:
            ...
            for lr in q("""SELECT FROM_WIDGET_ID,FROM_FIELD_NAME,TO_WIDGET_ID,TO_FIELD_NAME
                ...
                # Force resolution: register stub instances for any widget
                # not yet in the instances dict (e.g. reusable widgets)
                for inst_name, wid_key in ((from_inst, lr["FROM_WIDGET_ID"]),
                                           (to_inst,   lr["TO_WIDGET_ID"])):
                    if inst_name and inst_name not in m.instances:
                        m.instances[inst_name] = MappingInstance(
                            name=inst_name,
                            transformation_name=inst_name,
...
def main():
    ...
    ap.add_argument("--src-dialect", default="duckdb", choices=DB_DIALECTS,
                    help="Source database dialect (default: duckdb)")
    ap.add_argument("--tgt-dialect", default="duckdb", choices=DB_DIALECTS,
                    help="Target/dbt database dialect (default: duckdb)")
    args = ap.parse_args()
    out  = Path(args.out)
