...
class Transformation:
    ...
    # --- SQL override properties ---
    @property
    def pre_sql(self): return self.attributes.get("Pre SQL", self.attributes.get("Pre-load SQL", ""))
    @property
    def post_sql(self): return self.attributes.get("Post SQL", self.attributes.get("Post-load SQL", ""))
    @property
    def insert_sql(self): return self.attributes.get("Insert SQL Override", self.attributes.get("Insert Override", ""))
    @property
    def update_sql(self): return self.attributes.get("Update SQL Override", self.attributes.get("Update Override", ""))
    @property
    def delete_sql(self): return self.attributes.get("Delete SQL Override", self.attributes.get("Delete Override", ""))
    @property
    def target_load_type(self): return self.attributes.get("Target Load Type", "Normal")
    @property
    def truncate_target(self): return self.attributes.get("Truncate Target Table", "NO").upper() == "YES"
...
@dataclass
class SqlOverrideBundle:
    """
    All SQL overrides extracted from a single mapping, keyed by instance name.
    Each list entry is (instance_name, transformation_type, sql_text, override_kind).
    override_kind is one of: pre_sql | post_sql | insert | update | delete |
                              sq_query | sq_filter | sq_join | lookup_sql |
                              lookup_cond | filter_cond | join_cond | sp_call
    """
    mapping_name: str
    overrides: list[tuple[str, str, str, str]] = field(default_factory=list)

    def add(self, inst: str, ttype: str, sql: str, kind: str):
        if sql and sql.strip():
            self.overrides.append((inst, ttype, sql.strip(), kind))

    @property
    def has_pre_post(self) -> bool:
        return any(k in ("pre_sql","post_sql") for _,_,_,k in self.overrides)

    @property
    def has_dml(self) -> bool:
        return any(k in ("insert","update","delete") for _,_,_,k in self.overrides)
...
class Mapping:
    ...
    def collect_sql_overrides(self) -> "SqlOverrideBundle":
        """
        Walk every transformation in the mapping and collect all SQL override
        attributes into a single SqlOverrideBundle for the generator to use.
        """
        bundle = SqlOverrideBundle(mapping_name=self.name)
        for inst_name, inst in self.instances.items():
            ...
            # Pre / Post SQL — can appear on any transformation that has
            # a connection (Source Qualifier, Target Definition, Stored Procedure)
            bundle.add(inst_name, tt, t.pre_sql,  "pre_sql")
            bundle.add(inst_name, tt, t.post_sql, "post_sql")

            # Source Qualifier overrides
            if tt in ("Source Qualifier", "Source Definition", "Input Transformation"):
                bundle.add(inst_name, tt, t.attributes.get("Sql Query",""),       "sq_query")
                bundle.add(inst_name, tt, t.attributes.get("Source Filter",""),   "sq_filter")
                bundle.add(inst_name, tt, t.attributes.get("User Defined Join",""),"sq_join")

            # Lookup overrides
            if tt == "Lookup Procedure":
                bundle.add(inst_name, tt, t.lookup_sql,       "lookup_sql")
                bundle.add(inst_name, tt, t.lookup_condition, "lookup_cond")

            # Filter
            if tt == "Filter":
                bundle.add(inst_name, tt, t.filter_condition, "filter_cond")

            # Joiner
            if tt == "Joiner":
                bundle.add(inst_name, tt,
                           t.attributes.get("Join Condition",""), "join_cond")

            # Target DML overrides
            if tt in ("Target Definition", "Output Transformation"):
                bundle.add(inst_name, tt, t.insert_sql, "insert")
                bundle.add(inst_name, tt, t.update_sql, "update")
                bundle.add(inst_name, tt, t.delete_sql, "delete")

            # Stored procedure call text
            if tt == "Stored Procedure":
                bundle.add(inst_name, tt,
                           t.attributes.get("Call Text",""), "sp_call")
...
class PowerMartParser:
    ...
    def parse_file(self, xml_path: str) -> tuple[list[Mapping], list[Workflow]]:
        ...
        # ------------------------------------------------------------------
        # Pass 1: collect ALL transformations across ALL folders first.
        # This lets us resolve reusable mapplets and shortcuts that reference
        # objects defined in a different folder within the same XML export.
        # ------------------------------------------------------------------
        all_transformations: dict[str, Transformation] = {}   # global_key -> Transformation
        # global_key = "<FOLDER>/<TRANSFORMATION_NAME>"
        for repo in root.iter("REPOSITORY"):
            for folder in repo.iter("FOLDER"):
                fn = folder.get("NAME", "")
                for t_el in folder.iter("TRANSFORMATION"):
                    trans = self._parse_transformation(t_el)
                    key   = f"{fn}/{trans.name}"
                    all_transformations[key] = trans

        # ------------------------------------------------------------------
        # Pass 2: build shortcut resolution map.
        # SHORTCUT elements reference objects (mappings/mapplets) by name in
        # another folder.  We index them so that when an INSTANCE refers to a
        # reusable transformation that isn't in the local folder, we can look
        # it up from the referenced folder.
        # ------------------------------------------------------------------
        shortcuts: dict[str, str] = {}   # "<local_folder>/<name>" -> "<remote_folder>/<name>"
        for repo in root.iter("REPOSITORY"):
            for folder in repo.iter("FOLDER"):
                fn = folder.get("NAME", "")
                for sc in folder.findall("SHORTCUT"):
                    sc_name    = sc.get("NAME", "")
                    ref_folder = sc.get("FOLDERNAME", sc.get("REFERENCEFOLDER", ""))
                    ref_name   = sc.get("REFOBJECTNAME", sc_name)
                    if sc_name and ref_folder:
                        local_key  = f"{fn}/{sc_name}"
                        remote_key = f"{ref_folder}/{ref_name}"
                        shortcuts[local_key] = remote_key

        # ------------------------------------------------------------------
        # Pass 3: parse mappings / mapplets / workflows normally, but use
        # all_transformations + shortcuts to resolve reusable instances.
        # ------------------------------------------------------------------
        for repo in root.iter("REPOSITORY"):
            for folder in repo.iter("FOLDER"):
                fn = folder.get("NAME", "")
                for el in folder.iter("MAPPLET"):
                    m = self._parse_mapping_el(
                        el, fn, True, all_transformations, shortcuts)
                    if m:
                        mappings.append(m)
                for el in folder.findall("MAPPING"):
                    m = self._parse_mapping_el(
                        el, fn, False, all_transformations, shortcuts)
                    if m:
                        mappings.append(m)
    ...
    # ------------------------------------------------------------------
    # Helper: resolve a transformation for an INSTANCE element.
    # Priority:
    #   1. Local transformation (same mapping XML element)
    #   2. Shortcut -> remote folder transformation
    #   3. Any transformation with matching name in all_transformations
    # ------------------------------------------------------------------
    @staticmethod
    def _resolve_transformation(
        ...
        trans_name: str,
        folder: str,
        local_transforms: dict[str, Transformation],
        all_transforms:   dict[str, Transformation],
        shortcuts:        dict[str, str],
    ) -> Optional[Transformation]:
        # 1. Local
        if trans_name in local_transforms:
            return local_transforms[trans_name]
        # 2. Shortcut from this folder
        local_key = f"{folder}/{trans_name}"
        if local_key in shortcuts:
            remote_key = shortcuts[local_key]
            if remote_key in all_transforms:
                return all_transforms[remote_key]
        # 3. Global search by name (handles reusable mapplets from any folder)
        for key, trans in all_transforms.items():
            if key.endswith(f"/{trans_name}"):
                return trans
        return None

    def _parse_mapping_el(self, el: ET.Element, folder: str, is_mapplet: bool,
                           all_transforms: Optional[dict] = None,
                           shortcuts:      Optional[dict] = None) -> Optional[Mapping]:
        m = Mapping(name=el.get("NAME",""), folder=folder,
                    description=el.get("DESCRIPTION",""), is_mapplet=is_mapplet)

        # Local transformations defined inside this mapping/mapplet element
        local_transforms: dict[str, Transformation] = {}
        for t in el.findall("TRANSFORMATION"):
            trans = self._parse_transformation(t)
            local_transforms[trans.name] = trans
        ...
        for inst in el.findall("INSTANCE"):
            ...
            # Resolve transformation body — may live in another folder
            resolved = self._resolve_transformation(
                inst_name, trans_name, folder,
                local_transforms,
                all_transforms or {},
                shortcuts or {},
            )
            if resolved and trans_name not in m.transformations:
                # Register the resolved (cross-folder) transformation body
                m.transformations[trans_name] = resolved
                origin = "shortcut/reusable"
                if f"{folder}/{trans_name}" in (shortcuts or {}):
                    remote = (shortcuts or {})[f"{folder}/{trans_name}"]
                    origin = f"shortcut -> {remote}"
                # Annotate so the generator can note cross-folder references
                resolved.attributes.setdefault("__origin__", origin)

            m.instances[inst_name] = MappingInstance(
                name=inst_name,
                transformation_name=trans_name,
                transformation_type=trans_type or (resolved.ttype if resolved else "Unknown"),
                reusable=reusable,
            )

        # Synthesise instances from local transformations if no INSTANCE elements
        if not m.instances:
            for tname, trans in local_transforms.items():
                m.instances[tname] = MappingInstance(tname, tname, trans.ttype)

        # Connector evaluation with eager endpoint registration
        for c_el in el.findall("CONNECTOR"):
            ...
            for inst_name, trans_name, trans_type in (
                ...
                if inst_name and inst_name not in m.instances:
                    resolved = self._resolve_transformation(
                        inst_name, trans_name or inst_name, folder,
                        local_transforms, all_transforms or {}, shortcuts or {},
                    )
                    if resolved and (trans_name or inst_name) not in m.transformations:
                        m.transformations[trans_name or inst_name] = resolved
                    m.instances[inst_name] = MappingInstance(
                        name=inst_name,
...
class DbtGenerator:
    ...
    def _write_mapping_models(self, m: Mapping) -> tuple[list[dict], list[dict]]:
        ...
        # Collect all SQL overrides for this mapping
        bundle = m.collect_sql_overrides()

        base_name = f"{'mplt' if m.is_mapplet else 'stg'}_{_snake(m.name)}"
        base_sql  = self._mapping_base_sql(m, base_name, bundle)
        ...
        # Write macro file for any Pre/Post SQL or DML overrides
        if bundle.overrides:
            self._write_override_macros(m, bundle)
        ...
        for idx, pair in enumerate(m.source_target_pairs):
            ...
            if pair.is_scd2:
                ...
                sql = self._pair_model_sql(m, pair, pair_name, base_name, bundle)
                layer = "staging"
    ...
    def _mapping_base_sql(self, m: Mapping, model_name: str,
                           bundle: "SqlOverrideBundle | None" = None) -> str:
        ordered  = m.topo_sort()
        ...
        # Build pre/post hook config from bundle
        pre_hooks, post_hooks = [], []
        if bundle:
            for inst, ttype, sql, kind in bundle.overrides:
                translated = self._tv(sql)
                if kind == "pre_sql":
                    pre_hooks.append(translated.replace('"', '\\"').replace("\n", " "))
                elif kind == "post_sql":
                    post_hooks.append(translated.replace('"', '\\"').replace("\n", " "))

        hook_lines = []
        if pre_hooks:
            hook_lines.append("    pre_hook  = [")
            hook_lines.extend(f'        "{h}",' for h in pre_hooks)
            hook_lines.append("    ],")
        if post_hooks:
            hook_lines.append("    post_hook = [")
            hook_lines.extend(f'        "{h}",' for h in post_hooks)
            hook_lines.append("    ],")
        ...
        header = textwrap.dedent(f"""\
            {{# -----------------------------------------------------------
               ...
               SQL overrides: {len(bundle.overrides) if bundle else 0}
            ----------------------------------------------------------- #}}
            {{{{ config(
                materialized='view'{(',' + hook_block) if hook_block else ''}
            ) }}}}
        """)
    ...
    def _pair_model_sql(self, m: Mapping, pair: SourceTargetPair,
                         model_name: str, base_ref: str,
                         bundle: "SqlOverrideBundle | None" = None) -> str:
        """
        Source->target pair model. Inlines translated DML overrides (INSERT/UPDATE/DELETE)
        as commented SQL blocks and wires pre/post hooks via config().
        """
        upd = pair.update_strategy.upper()
        if "DD_DELETE" in upd:
            strategy_comment = "-- DELETE strategy: rows flagged for deletion"
            dml_note = "-- DuckDB: implement via MERGE or soft-delete flag"
        elif "DD_UPDATE" in upd:
            strategy_comment = "-- UPDATE strategy"
            dml_note = "-- DuckDB: implement via MERGE INTO or INSERT ... ON CONFLICT"
        else:
            strategy_comment = ""; dml_note = ""

        # Collect DML overrides that belong to the target instance in this pair
        insert_blocks, update_blocks, delete_blocks = [], [], []
        pre_hooks, post_hooks = [], []
        if bundle:
            tgt_inst = pair.target_instance
            for inst, ttype, sql, kind in bundle.overrides:
                translated = self._tv(sql)
                if inst == tgt_inst or ttype in ("Target Definition","Output Transformation"):
                    if kind == "insert": insert_blocks.append(translated)
                    elif kind == "update": update_blocks.append(translated)
                    elif kind == "delete": delete_blocks.append(translated)
                if kind == "pre_sql" and inst == tgt_inst:
                    pre_hooks.append(translated.replace('"', '\\"').replace("\n", " "))
                if kind == "post_sql" and inst == tgt_inst:
                    post_hooks.append(translated.replace('"', '\\"').replace("\n", " "))

        def _sql_block(label: str, stmts: list[str]) -> str:
            if not stmts: return ""
            lines = [f"\n-- ===== {label} OVERRIDE ====="]
            for s in stmts:
                lines.append("/*")
                lines.extend(f"  {ln}" for ln in s.splitlines())
                lines.append("*/")
            return "\n".join(lines)
        ...
        dml_section = (
            _sql_block("INSERT", insert_blocks)
            + _sql_block("UPDATE", update_blocks)
            + _sql_block("DELETE", delete_blocks)
        )
        ...
        return textwrap.dedent(f"""\
            ...
            ----------------------------------------------------------- #}}
            {{{{ config(
                materialized='table'{(',' + hook_block) if hook_block else ''}
            ) }}}}
    ...
    # ------------------------------------------------------------------
    # Macro file: one macro per mapping that has SQL overrides
    # ------------------------------------------------------------------
    def _write_override_macros(self, m: Mapping, bundle: SqlOverrideBundle):
        """
        Generate a macro file for all SQL overrides in a mapping.
        Each override kind gets its own named macro so it can be called
        from models or orchestration tools independently.

        Macro naming: infa_<mapping_snake>__<inst_snake>__<kind>()
        """
        macro_name = _snake(m.name)
        lines = [
            f"{{# Auto-generated SQL override macros for mapping: {m.name} #}}",
            f"{{# Source folder: {m.folder} #}}",
            "",
        ]

        for inst, ttype, sql, kind in bundle.overrides:
            translated = self._tv(sql)
            fn_name    = f"infa_{macro_name}__{_snake(inst)}__{kind}"
            # Build a comment header showing original vs translated
            orig_lines = "\n".join(f"  -- {ln}" for ln in sql.strip().splitlines()[:20])
            lines += [
                f"{{%- macro {fn_name}() -%}}",
                f"  {{# Override kind : {kind} #}}",
                f"  {{# Instance      : {inst} ({ttype}) #}}",
                f"  {{# Original SQL  :",
                orig_lines,
                f"  #}}",
                "  " + translated.replace("\n", "\n  "),
        ...
        macro_path = self.out / "macros" / f"overrides_{macro_name}.sql"
        _write(macro_path, "\n".join(lines))

    # ------------------------------------------------------------------
    # Updated _instance_to_cte: expose ALL SQL override attributes inline
    # ------------------------------------------------------------------
    ...
    def _instance_to_cte(self, inst_name, inst, t, upstream, m) -> str:
        ...
        if t.ttype in ("Input Transformation","Source Qualifier","Source Definition"):
            ...
            sq_override = self.xp.transpile(sq_raw)
            src = self._tv(t.attributes.get("Table Name",
                           t.attributes.get("Source table name", inst_name)))
            src_ref = f"{{{{ source('raw', '{_snake(src)}') }}}}"

            # Pre SQL as leading comment (full text exposed)
            pre  = self._tv(t.pre_sql).strip()
            post = self._tv(t.post_sql).strip()
            pre_block  = f"\n    -- PRE SQL:\n    -- {pre.replace(chr(10), chr(10)+chr(32)*4+'-- ')}" if pre else ""
            post_block = f"\n    -- POST SQL:\n    -- {post.replace(chr(10), chr(10)+chr(32)*4+'-- ')}" if post else ""

            # User-defined join inline
            ud_join = self._tv(t.attributes.get("User Defined Join","")).strip()
            join_clause = f"\n    -- USER DEFINED JOIN: {ud_join}" if ud_join else ""

            if sq_override and len(sq_override) > 5:
                return (f"{n} as ({pre_block}\n"
                        f"    -- Source Qualifier SQL override\n"
                        f"    {sq_override}{post_block}\n)")
            return (f"{n} as ({pre_block}\n"
                    f"    select {cols} from {src_ref}"
                    f"{join_clause}{post_block}\n)")
        ...
        if t.ttype in ("Output Transformation","Target Definition"):
            ...
            # Expose all DML overrides as commented SQL blocks
            pre    = self._tv(t.pre_sql).strip()
            post   = self._tv(t.post_sql).strip()
            ins    = self._tv(t.insert_sql).strip()
            upd_s  = self._tv(t.update_sql).strip()
            del_s  = self._tv(t.delete_sql).strip()
            trunc  = "-- TRUNCATE TARGET before load" if t.truncate_target else ""

            def _block(label: str, sql: str) -> str:
                if not sql: return ""
            ...
            return (f"{n} as (\n"
                    f"    {trunc}"
                    + _block("PRE SQL",    pre)
                    + _block("INSERT SQL", ins)
                    + _block("UPDATE SQL", upd_s)
                    + _block("DELETE SQL", del_s)
                    + f"\n    select {cols}\n    from {up_n[0]}"
                    + _block("POST SQL",   post)
                    + "\n)")
        ...
        if t.ttype == "Stored Procedure":
            ...
            call_txt = self._tv(t.attributes.get("Call Text","")).strip()
            pre     = self._tv(t.pre_sql).strip()
            post    = self._tv(t.post_sql).strip()
            call_block = (f"\n    -- CALL TEXT:\n    --   {call_txt.replace(chr(10), chr(10)+'    --   ')}"
                          if call_txt else "")
            pre_block  = f"\n    -- PRE SQL:\n    --   {pre}"  if pre  else ""
            post_block = f"\n    -- POST SQL:\n    --   {post}" if post else ""
            return (f"{n} as ({pre_block}\n"
                    f"    -- SP: {t.sp_name}({ic})  conn:{conn}\n"
                    f"    -- TODO: implement as DuckDB macro / UDF{call_block}\n"
                    f"    select src.*, {oc or 'null as proc_result'}\n"
                    f"    from {fc} src{post_block}\n)")
    ...
    def _write_lineage_report(self, mappings, taskflows, workflows,
        ...
        for m in mappings:
            ...
            bundle = m.collect_sql_overrides()
            if bundle.overrides:
                lines += ["| Instance | Type | Kind | SQL (first 120 chars) |",
                          "|----------|------|------|-----------------------|"]
                for inst, ttype, sql, kind in bundle.overrides:
                    preview = sql.replace("\n"," ")[:120]
                    lines.append(f"| `{inst}` | {ttype} | `{kind}` | `{preview}` |")
                if bundle.has_pre_post:
                    lines.append("\n> Pre/Post SQL wired as dbt `pre_hook`/`post_hook` in model config.")
                if bundle.has_dml:
                    lines.append(f"\n> DML overrides exposed inline in model and in `macros/overrides_{_snake(m.name)}.sql`.")
            else:
                lines.append("_No SQL overrides found._")
            lines.append("")
            lines.append("\n**Expressions:**\n")
...
# ===========================================================================
class PCSOAPClient:
    # Common SOAP service paths — tried in order until one succeeds
    _SERVICE_PATHS = [
        "/wsh/services/RepositoryService",
        "/services/RepositoryService",
        "/wsh/services/Repository",
        "/PowerCenterServices/RepositoryService",
    ]

    def __init__(self, host, port, repo, user, password,
                 domain="Native", use_https=False, service_path=None):
        self.base     = f"{'https' if use_https else 'http'}://{host}:{port}"
        self.repo     = repo
        self.user     = user
        self.password = password
        self.domain   = domain          # auth domain: Native | LDAP | Kerberos
        self._token   = ""
        self._sess    = None
        # Allow explicit override; otherwise auto-discover
    ...
    def connect(self):
        try:
            ...
            # Disable SSL verification warnings if connecting via HTTP
        ...
        if self._service_path:
            ...
            paths = self._SERVICE_PATHS

        last_err = None
        for path in paths:
            self._url = f"{self.base}{path}"
            print(f"  [PC SOAP] Trying: {self._url}")
            try:
                self._token = self._login()
                print(f"  [PC SOAP] Connected: {self._url}  repo={self.repo}  domain={self.domain}")
                return
        ...
        raise RuntimeError(
            f"Could not connect to PowerCenter Repository Service.\n"
            f"Tried paths: {paths}\n"
            f"Last error : {last_err}\n\n"
            f"Troubleshooting:\n"
            f"  1. Verify port with: netstat -an | findstr {self.base.split(':')[-1]}\n"
            f"  2. Try --pc-https if TLS is required\n"
            f"  3. Try --pc-domain LDAP if your org uses LDAP auth\n"
            f"  4. Try --pc-service-path /services/RepositoryService\n"
            f"  5. Check pmrep.log on the server for the actual endpoint"
        )

    def _login(self) -> str:
        """
        Build a Login SOAP envelope.  PowerCenter 9.x / 10.x accepts
        both the v1 (RepositoryDomainName) and v2 (SecurityDomain) element
        names — we send both so either server version accepts the request.
        """
        body = (
            f"<irs:Login xmlns:irs='http://www.informatica.com/wsh'>"
            f"  <irs:RepositoryName>{self.repo}</irs:RepositoryName>"
            f"  <irs:UserName>{self.user}</irs:UserName>"
            f"  <irs:Password>{self.password}</irs:Password>"
            f"  <irs:RepositoryDomainName>{self.domain}</irs:RepositoryDomainName>"
            f"  <irs:SecurityDomain>{self.domain}</irs:SecurityDomain>"
            f"</irs:Login>"
        )
        resp  = self._post("Login", body)
        token = resp.find(".//{*}SessionToken")
        if token is None or not (token.text or "").strip():
            raise RuntimeError(
                f"Login succeeded HTTP-wise but returned no session token.\n"
                f"Response: {self._last_raw[:400]}"
            )
    ...
    def _build_envelope(self, body: str) -> str:
        return (
            ...
            ' xmlns:irs="http://www.informatica.com/wsh"'
    ...
    def _post(self, action: str, body: str):
        import xml.etree.ElementTree as ET
        envelope = self._build_envelope(body)
        headers  = {
            "Content-Type": "text/xml; charset=utf-8",
            "SOAPAction":   f'"http://www.informatica.com/wsh/{action}"',
        }
        r = self._sess.post(
            self._url,
        ...
        if r.status_code >= 400:
            # Try to extract a SOAP fault message for a clearer error
            try:
                root  = ET.fromstring(r.content)
                fault = root.find(".//{http://schemas.xmlsoap.org/soap/envelope/}Fault")
                if fault is not None:
                    fstr = ET.tostring(fault, encoding="unicode")
                    raise RuntimeError(f"SOAP Fault ({r.status_code}): {fstr[:300]}")
            except ET.ParseError:
                pass
            r.raise_for_status()
        root     = ET.fromstring(r.content)
    ...
    def list_folders(self) -> list[str]:
        body = (
            f"<irs:GetAllFolders>"
            f"  <irs:SessionToken>{self._token}</irs:SessionToken>"
            f"</irs:GetAllFolders>"
        )
    ...
    def export_folder_xml(self, folder: str) -> bytes:
        body = (
            f"<irs:ExportObjects>"
            f"  <irs:SessionToken>{self._token}</irs:SessionToken>"
            f"  <irs:ObjectsToExport>"
            f"    <irs:FolderName>{folder}</irs:FolderName>"
            f"    <irs:ObjectTypes>mapping</irs:ObjectTypes>"
            f"    <irs:ObjectTypes>mapplet</irs:ObjectTypes>"
            f"    <irs:ObjectTypes>workflow</irs:ObjectTypes>"
            f"  </irs:ObjectsToExport>"
            f"  <irs:DependencyOptions>"
            f"    <irs:AddDependency>false</irs:AddDependency>"
            f"  </irs:DependencyOptions>"
            f"</irs:ExportObjects>"
        )
...
class PCFetcher:
    def __init__(self, args, out_dir: Path):
        self.args    = args
        self.out     = out_dir
        self.raw     = out_dir / "_pc_raw"
        self.raw.mkdir(parents=True, exist_ok=True)
        self.pm      = PowerMartParser()
        self.par     = ParameterFileParser()
        # Build folder filter from --pc-folder (comma-separated, wildcard-aware)
        raw_folders  = getattr(args, "pc_folder", None) or ""
        self.folders: list[str] = [f.strip() for f in raw_folders.split(",") if f.strip()]
        if self.folders:
            print(f"  [PC] Folder filter: {self.folders}")

    def _folder_match(self, folder_name: str) -> bool:
        """
        Return True if folder_name matches any entry in self.folders.
        Matching rules (in order):
          1. No filter specified  -> always True
          2. Exact match          -> True
          3. Shell-style wildcard -> fnmatch (e.g. 'MEDICA_*', '*_REUSABLE')
          4. Case-insensitive exact match as fallback
        """
        if not self.folders:
            return True
        import fnmatch
        for pattern in self.folders:
            ...
            if fnmatch.fnmatch(folder_name, pattern):
                return True
            if folder_name.upper() == pattern.upper():
                return True
        return False
    def fetch_all(self):
    ...
    def _soap(self):
        client = PCSOAPClient(
            ...
            domain       = getattr(self.args, "pc_domain",       "Native"),
            use_https    = getattr(self.args, "pc_https",         False),
            service_path = getattr(self.args, "pc_service_path",  None),
        )
        client.connect()
        all_m, all_w = [], []
        try:
            all_folders = client.list_folders()
            matched     = [f for f in all_folders if self._folder_match(f)]
            skipped     = len(all_folders) - len(matched)
            print(f"  [PC SOAP] {len(all_folders)} folder(s) found, "
                  f"{len(matched)} match filter, {skipped} skipped")

            # Export matched folders and collect raw XML bytes
            folder_xmls: dict[str, bytes] = {}
            for folder in matched:
                print(f"    Exporting folder: {folder}")
                try:
                    xb = client.export_folder_xml(folder)
                    folder_xmls[folder] = xb
                    xp = self.raw / f"{_snake(folder)}.xml"
                    xp.write_bytes(xb)
                except Exception as e:
                    print(f"      Warning {folder}: {e}")

            # Discover which additional (shared/reusable) folders are needed
            # by scanning SHORTCUT elements in the exported XML files
            import xml.etree.ElementTree as _ET
            needed_extra: set[str] = set()
            for folder, xb in folder_xmls.items():
                try:
                    root = _ET.fromstring(xb)
                    for sc in root.iter("SHORTCUT"):
                        ref = sc.get("FOLDERNAME", sc.get("REFERENCEFOLDER",""))
                        if ref and ref not in folder_xmls and ref not in needed_extra:
                            needed_extra.add(ref)
                except Exception:
                    pass

            if needed_extra:
                print(f"  [PC SOAP] Fetching {len(needed_extra)} additional "
                      f"shared/reusable folder(s): {sorted(needed_extra)}")
                for folder in sorted(needed_extra):
                    if folder not in [f for f in all_folders]:
                        print(f"    Skipping unknown folder: {folder}")
                        continue
                    print(f"    Exporting shared folder: {folder}")
                    try:
                        xb = client.export_folder_xml(folder)
                        folder_xmls[folder] = xb
                        xp = self.raw / f"{_snake(folder)}_shared.xml"
                        xp.write_bytes(xb)
                    except Exception as e:
                        print(f"      Warning {folder}: {e}")

            # Parse all collected XMLs together so cross-folder resolution works
            combined_xml_paths = [
                str(self.raw / f"{_snake(f)}.xml")
                for f in matched if (self.raw / f"{_snake(f)}.xml").exists()
            ] + [
                str(self.raw / f"{_snake(f)}_shared.xml")
                for f in needed_extra if (self.raw / f"{_snake(f)}_shared.xml").exists()
            ]

            # Use a single merged parse so PowerMartParser can do cross-folder
            # shortcut + reusable resolution across all files at once
            merged_xml = self.raw / "_merged_export.xml"
            self._merge_xml_files(combined_xml_paths, merged_xml)
            ms, wfs = self.pm.parse_file(str(merged_xml))

            # Filter back to only mappings from matched folders
            ms  = [m for m in ms  if self._folder_match(m.folder)]
            wfs = [w for w in wfs if self._folder_match(w.folder)]
            all_m.extend(ms)
            all_w.extend(wfs)
            print(f"  [PC SOAP] {len(all_m)} mapping(s), {len(all_w)} workflow(s) after resolution")
    ...
    @staticmethod
    def _merge_xml_files(paths: list[str], out_path: Path):
        """
        Merge multiple PowerMart XML exports into one so the parser
        can resolve cross-folder references in a single pass.
        All FOLDER elements are collected under one REPOSITORY root.
        """
        import xml.etree.ElementTree as _ET
        folders_seen: set[str] = set()
        root_out = _ET.Element("POWERMART", CREATION_DATE="merged", REPOSITORY_VERSION="merged")
        repo_out = _ET.SubElement(root_out, "REPOSITORY",
                                  NAME="merged", VERSION="merged",
        ...
        for path in paths:
            ...
            for repo in tree.getroot().iter("REPOSITORY"):
                for folder in repo.iter("FOLDER"):
                    ...
                    if fname not in folders_seen:
                        folders_seen.add(fname)
                        repo_out.append(folder)
        _ET.ElementTree(root_out).write(str(out_path),
                                        encoding="utf-8", xml_declaration=True)
    def _db(self):
        try:
            from sqlalchemy import create_engine, text
        except ImportError:
            raise RuntimeError("pip install sqlalchemy cx_Oracle")
        ...
        # Build folder WHERE clause from filter
        if self.folders:
            # Use LIKE patterns for wildcard support in SQL
            import fnmatch
            like_clauses = []
            exact_names  = []
            for pat in self.folders:
                if "*" in pat or "?" in pat:
                    sql_like = pat.replace("*", "%").replace("?", "_")
                    like_clauses.append(f"UPPER(s.SUBJ_NAME) LIKE '{sql_like.upper()}'")
                else:
                    exact_names.append(pat.upper())
            conditions = list(like_clauses)
            if exact_names:
                quoted = ", ".join(f"'{n}'" for n in exact_names)
                conditions.append(f"UPPER(s.SUBJ_NAME) IN ({quoted})")
            folder_where = "AND (" + " OR ".join(conditions) + ")"
        else:
            folder_where = ""
        ...
        """
        mapping_rows = q(mapping_sql)
        print(f"  [PC DB] {len(mapping_rows)} mapping(s) found"
              + (f" in folder(s): {self.folders}" if self.folders else " (all folders)"))

        all_m = []
        for row in mapping_rows:
            mid = row["MAPPING_ID"]
            m   = Mapping(
                name=row["MAPPING_NAME"], folder=row["FOLDER_NAME"],
                description=row.get("COMMENTS",""),
                is_mapplet=(row["OBJ_TYPE"]=="MAPPLET"),
            )
            wids = {}
            for wr in q("SELECT WIDGET_ID,WIDGET_NAME,WIDGET_TYPE_NAME,REUSABLE,DESCRIPTION "
                        "FROM OPB_WIDGET WHERE MAPPING_ID=:mid", {"mid": mid}):
                ...
                m.transformations[wn] = t
                m.instances[wn]       = MappingInstance(wn, wn, t.ttype)
            for fr in q("""SELECT WIDGET_ID,FIELD_NAME,DATATYPE,PORTTYPE,DEFAULT_VALUE,
                               DESCRIPTION,PRECISION,SCALE,EXPRESSION
                           FROM OPB_WIDGET_FIELD
                           WHERE WIDGET_ID IN
                               (SELECT WIDGET_ID FROM OPB_WIDGET WHERE MAPPING_ID=:mid)""",
                        {"mid": mid}):
            ...
            for lr in q("""SELECT FROM_WIDGET_ID,FROM_FIELD_NAME,TO_WIDGET_ID,TO_FIELD_NAME
                ...
                for inst_name, _ in ((from_inst, lr["FROM_WIDGET_ID"]),
        ...
        conns = [
            {"id": str(r.get("OBJECT_ID","")), "name": r.get("OBJECT_NAME",""),
             "type": r.get("OBJECT_TYPE_NAME",""), "host": r.get("SERVER_NAME",""),
             "port": str(r.get("PORT_NO","")), "database": r.get("DATABASE_NAME","")}
            for r in q("SELECT OBJECT_ID,OBJECT_NAME,OBJECT_TYPE_NAME,"
                       "SERVER_NAME,PORT_NO,DATABASE_NAME FROM OPB_CNX")
        ]
    ...
    def _xml(self):
        ...
        for f in files:
            try:
                ms, wfs = self.pm.parse_file(f)
                # Apply folder filter to parsed mappings
                if self.folders:
                    ms  = [m for m in ms  if self._folder_match(m.folder)]
                    wfs = [w for w in wfs if self._folder_match(w.folder)]
                all_m.extend(ms)
                ...
                print(f"    Warning {f}: {e}")
        if self.folders:
            print(f"  [PC XML] {len(all_m)} mapping(s) after folder filter {self.folders}")
        return all_m, all_w, self._infer_conns(all_w), self._params()
    def _params(self):
...
def main():
    ...
    ap.add_argument("--pc-domain",   default=os.environ.get("PC_DOMAIN","Native"),
                    help="Auth domain for SOAP login: Native | LDAP | Kerberos (default: Native)")
    ap.add_argument("--pc-service-path", default=None, metavar="PATH",
                    help="Override SOAP service path, e.g. /services/RepositoryService")
    ap.add_argument("--pc-db-url");  ap.add_argument("--pc-params", metavar="PATH")
    ap.add_argument("--pc-folder",   metavar="FOLDER", default=None,
                    help="Start extraction from this repository folder (or comma-separated list). "
                         "SOAP: exports only matching folders. "
                         "DB: filters OPB_MAPPING by SUBJ_NAME. "
                         "XML: filters parsed mappings by folder name. "
                         "Supports shell-style wildcards: 'MEDICA_*'  (default: all folders)")
    ...
    if args.idmc:
        ...
        # Folder filter for local XML mode
        import fnmatch as _fnmatch
        folder_patterns = [f.strip() for f in (getattr(args,"pc_folder",None) or "").split(",") if f.strip()]

        def _local_folder_match(folder_name: str) -> bool:
            if not folder_patterns:
                return True
            for pat in folder_patterns:
                if folder_name == pat: return True
                if _fnmatch.fnmatch(folder_name, pat): return True
                if folder_name.upper() == pat.upper(): return True
            return False
        ...
        for xf in xml_files:
            ...
            try:
                ...
                # Apply folder filter
                if folder_patterns:
                    ms  = [m for m in ms  if _local_folder_match(m.folder)]
                    wfs = [w for w in wfs if _local_folder_match(w.folder)]
                for m in ms:
